...
import { TariffRequest } from "@/types/tariffs";
...
export function CreateTariffModal({
  ...
  const { createTariff } = useTariffs({});

  const [formData, setFormData] = useState<TariffRequest>({
    name: "",
    price: "",
    description: "",
    duration_days: 30,
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Tarif yaratishda xatolik yuz berdi"
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-lg font-semibold text-gray-900">
              {ready ? t("tariffs.create.title") : "Yangi tarif yaratish"}
            </h2>
          </div>
          <Button
            variant="ghost"
          ...
          >
            {loading ? (
              <div className="flex items-center space-x-2">
                ...
                <span>{ready ? t("common.creating") : "Yaratilmoqda..."}</span>
              </div>
            ) : (
              <>
                <Save className="w-4 h-4 mr-2" />
                {ready ? t("common.create") : "Yaratish"}
              </>
            )}
          </Button>
        </div>
